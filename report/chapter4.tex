...
More precively, we will look for periodic patterns that represent an interval in time \(t\) of \(0.33\) seconds. 
If we see such patterns with a period of \(4t = 1.33\) seconds, they could possibly be caused by the fact that the CPU is performing the operations caused by our utility, and argue that we are in fact able to destinguish between such low level operations based purely on the acoustic fingerprint. 
