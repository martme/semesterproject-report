...
The results given in~\cite[Fig.~2]{DBLP:conf/crypto/GenkinST14} suggest that it is possible to observe a difference, even between the different microinstructions ADD, MUL and NOP, but the most visible difference is in a frequency range much higher than the hard limit imposed on us by the sampling rate of our setup.
...
We have conducted experiments using different configurations of hardware, and as we observe big variations in acoustic fingerprints for the different computers.
An interesting side note is how the acoustic emanations of the Lenovo T60p laptop is changing depending on if the computer is running on battery power or with a power adapter.
...
We chose to conduct experiments on a Lenovo laptop computer similar to what is used in~\cite{DBLP:conf/crypto/GenkinST14}, and also on an even older Dell laptop, as it was suggested that older processors had more explicit acoustic fingerprints.
